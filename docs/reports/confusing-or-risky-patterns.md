...
- 🟡 Shippo webhook writes `shippingStatus` and `shippingLog` on orders (`netlify/functions/shippoWebhook.ts`) while the order schema does not define those fields (`packages/sanity-config/src/schemaTypes/documents/order.tsx`), so Studio UX cannot surface them.
