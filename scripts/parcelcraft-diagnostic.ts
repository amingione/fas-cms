...
async function main() {
  ...
  try {
    ...
    if (shippingRates.data.length === 0) {
      ...
      shippingRates.data.forEach((rate) => {
        ...
        console.log(`        Amount: $${rate.fixed_amount?.amount ? rate.fixed_amount.amount / 100 : 'N/A'}`);
      });
  ...
  try {
    ...
    if (sessions.data.length === 0) {
      ...
      for (const session of sessions.data) {
        ...
        console.log(`   Shipping Cost: ${session.shipping_cost ? `$${session.shipping_cost.amount_total / 100}` : 'null'}`);
        console.log(`   Has Shipping Address Collection: ${!!session.shipping_address_collection}`);
        console.log(`   Invoice Creation: ${session.invoice_creation?.enabled ? 'Enabled' : 'Disabled'}`);
        ...
        if (session.line_items?.data) {
          for (const item of session.line_items.data) {
            ...
            if (product && !('deleted' in product)) {
              ...
              console.log(`       - shipping_required: ${metadata.shipping_required || 'missing ❌'}`);
              ...
              console.log(`       - dimensions: ${metadata.length ? `${metadata.length}x${metadata.width}x${metadata.height} ${metadata.dimension_unit}` : 'missing ❌'}`);
  ...
  console.log('  5. ✅ Ensure products have all metadata: shipping_required, weight, origin_country');
