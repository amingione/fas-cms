...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!vendor || !(vendor as any).passwordResetToken || !(vendor as any).passwordResetExpires) {
    ...
    const expiresAt = new Date((vendor as any).passwordResetExpires);

    if ((vendor as any).passwordResetToken !== hashedToken || Number.isNaN(expiresAt.getTime()) || expiresAt.getTime() < Date.now()) {
      return new Response(JSON.stringify({ message: 'Invalid or expired reset link.' }), {
        status: 400,
        headers: { 'content-type': 'application/json' }
      });
