export async function POST({ request }: { request: Request }) {
  ...
  const validateRes = await fetch(`https://${import.meta.env.PUBLIC_SANITY_PROJECT_ID}.api.sanity.io/v1/data/query/${import.meta.env.PUBLIC_SANITY_DATASET}?query=*[_id == "${productId}"][0]{_type}`, {
    headers: {
  ...
  const res = await fetch(`https://${import.meta.env.PUBLIC_SANITY_PROJECT_ID}.api.sanity.io/${import.meta.env.SANITY_API_VERSION}/data/mutate/${import.meta.env.PUBLIC_SANITY_DATASET}`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${import.meta.env.SANITY_API_TOKEN}`,
