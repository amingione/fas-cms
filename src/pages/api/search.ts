  ...
  try {
    const results = await sanityClient.fetch(
      `*[
        ...
        (
          _type != "product" ||
          (!(_id in path('drafts.**')) && defined(slug.current) && status == "active" && (productType == "service" || productType == "bundle" || productType == "physical" || featured == true))
        )
      ][0..24]{
        _id,
        _type,
