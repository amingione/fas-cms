...
export default defineType({
  ...
  fields: [
    ...
    defineField({name: 'snippet', title: 'Snippet', type: 'text', rows: 4, readOnly: true}),
    ...
    defineField({name: 'rawPayload', title: 'Raw Payload', type: 'text', rows: 6, readOnly: true}),
    defineField({name: 'readOnly', title: 'Read Only Mirror', type: 'boolean', readOnly: true, initialValue: true}),
  ],
  preview: {
    select: {
      title: 'title',
    ...
    prepare({title, domain, lastDetectedAt}: {title?: string; domain?: string; lastDetectedAt?: string}) {
      const when = lastDetectedAt ? new Date(lastDetectedAt).toLocaleDateString() : 'unknown date';
      return {
        title: title || '(Untitled mention)',
        subtitle: `${domain || 'unknown domain'} · ${when}`,
      };
    },
  },
});
